...
| Matching latency | 1.066μs p50, 85.4k ops/sec | 100ms–500ms |
...
The matching engine runs on a custom Rust CLOB with a 1.066μs p50 match latency at 85,400 operations per second. Benchmarks use a single-threaded, in-process setup to reflect the actual latency you experience, not a theoretical maximum.
