...
Benchmarks show 0.71 μs median match latency (p99: 0.75 μs, p99.9: 1.83 μs) and 725,000 operations per second, representing an 11.2× latency improvement over the Pulse baseline with a 90% reduction in p99.9 tail latency. An adverse selection toxicity scorer runs on-path per taker fill with a fixed-size ring buffer OFI accumulator, adding sub-50 ns overhead at p50.
