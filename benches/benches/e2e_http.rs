...
fn prepare_order(kp: &Keypair, slot: usize, nonce: u64) -> Vec<u8> {
    ...
    let msg = format!("vela:order:{}:{}:{}:{}:{}", ms.id, side_sign, price, ms.qty, nonce);
...
struct Stats {
    p50: u64, p99: u64, p999: u64, p9999: u64,
    min: u64, max: u64, mean_ns: u64, stddev_ns: u64,
    count: usize,
}
...
fn compute_stats(timings: &mut Vec<u64>) -> Stats {
    ...
    let variance = timings.iter().map(|&t| { let d = t as f64 - mean; d * d })
        .sum::<f64>() / n as f64;
    let pct = |p: usize| -> u64 {
    ...
    Stats {
        p50: pct(500), p99: pct(990), p999: pct(999),
        p9999: timings[((n as f64 * 0.9999) as usize).min(n - 1)],
        min: timings[0], max: timings[n - 1],
        mean_ns: mean as u64, stddev_ns: variance.sqrt() as u64, count: n,
    }
}
...
async fn scenario_concurrent(
    ...
    let nonces: Arc<Vec<AtomicU64>> = Arc::new(
...
async fn scenario_burst(
    ...
    for h in handles { h.await.unwrap(); }
    eprintln!("[e2e_http] Burst finished in {:.2}s", t_start.elapsed().as_secs_f64());
...
async fn run() {
    ...
    eprintln!("[e2e_http] Server on :{port}  setup={:.1}s", t0.elapsed().as_secs_f64());
    ...
    // -----------------------------------------------------------------------
    eprintln!("[e2e_http] S1 single-threaded — {WARMUP_SECS}s warmup + {MEASURE_SECS}s measure ...");
    ...
    for _ in 0..S1_KEYS { let _ = gen_keypair(&mut rng2); }
    let s2_keypairs: Arc<Vec<Keypair>> = Arc::new(
        (0..S2_KEYS).map(|_| gen_keypair(&mut rng2)).collect()
    );
    let _ = all_keypairs[s2_start..s2_end].len(); // silence lint

    let t_s2 = Instant::now();
    let s2 = scenario_concurrent(
    ...
    for _ in 0..(S1_KEYS + S2_KEYS) { let _ = gen_keypair(&mut rng3); }
    ...
    println!("Samples: single={} concurrent={} burst={}", s1.count, s2.count, s3.count);
    println!();
    println!("{:<22} {:>10} {:>10} {:>10} {:>10} {:>14}", "Scenario", "p50", "p99", "p99.9", "p99.99", "throughput");
    println!("{}", "-".repeat(82));
    for (label, s, tput) in [
        ...
        println!("{:<22} {:>8} µs {:>8} µs {:>8} µs {:>8} µs {:>10.0} ops/s",
            label,
            s.p50 / 1_000, s.p99 / 1_000, s.p999 / 1_000, s.p9999 / 1_000,
            tput);
    }
    println!();
    ...
    for (label, s) in [("single-threaded", &s1), ("concurrent-32", &s2), ("burst-1000", &s3)] {
        println!("  {:<22} min={:.0} µs  mean={:.0} µs  stddev={:.0} µs  max={:.0} µs",
            label,
            s.min as f64 / 1_000.0, s.mean_ns as f64 / 1_000.0,
            s.stddev_ns as f64 / 1_000.0, s.max as f64 / 1_000.0);
    }
}
